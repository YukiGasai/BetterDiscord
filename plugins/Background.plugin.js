...
class Background {
	...
	load() {
		...
		function readTextFile(file, callback) {
			let filepath
			if(bs == 'win32'){
				filepath = os.homedir() + "\\AppData\\Roaming\\BetterDiscord\\plugins\\" + file;
			}else if(bs == 'linux'){
				filepath = os.homedir()+ "/.config/BetterDiscord/plugins/" + file;
			}
		...
		if(bs == 'win32'){
			jspath    = os.homedir() + "\\AppData\\Roaming\\BetterDiscord\\plugins\\Background.plugin.js";
			filepath1 = os.homedir() + "\\AppData\\Roaming\\BetterDiscord\\plugins\\Background.config.json";
		...
		fs.exists(filepath2, function (exists) {
			if (!exists) {
				fs.writeFile(filepath2, ThemeString, { flag: 'wx' }, function (err, data) { })
			}
		});
	}
	start() {
		...
		function update() {
			...
			document.body.style.background = `url("${images.img[x].link}")  no-repeat center center fixed`;
		}
		update();
		function Pause() {
